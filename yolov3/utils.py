...
YOLO_V3_WEIGHTS = "D:/PELUSO/ITSligo/lectures_MENG/4-Symulation and Testing/assignments/assignment 2 - group/TensorFlow-2.x-YOLOv3-master/model_data/yolov3.weights"
...
def Load_Yolo_model():
    ...
    print("frame: ", YOLO_FRAMEWORK," type: ", YOLO_TYPE)
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        if YOLO_CUSTOM_WEIGHTS == False:
            ...
            yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE, CLASSES=("D:/PELUSO/ITSligo/lectures_MENG/4-Symulation and Testing/assignments/assignment 2 - group/TensorFlow-2.x-YOLOv3-master/model_data/coco/coco.names"))
...
def draw_bbox(image, CSV_file, bboxes, frame_idx, scores,CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    cv2.putText(image, str(frame_idx), (image_w-100, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                        2, Text_colors, 1, lineType=cv2.LINE_AA)
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            # put text on left upper corner   ##new
            try:
                Area_px = ((x2-x1)*(y2-y1))
                extra_label = " "+str(score) + ": Sc:{:.3f};".format(scores[i]) + " Ar:{:d}".format(Area_px)
                cv2.putText(image, extra_label, (30, 30+(i*18)), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                        fontScale, Text_colors, bbox_thick, lineType=cv2.LINE_AA)

                #write row in CSV   #new
                CSV_file.writerow([str(frame_idx), str(score),"{:.3f}".format(scores[i]),"{:d}".format(Area_px),
                    str(x1), str(y1), str(x2), str(y2),"{}".format(NUM_CLASS[class_ind])])
            except:
                print("skip")
...
def postprocess_boxes(pred_bbox, original_image, input_size, score_threshold):
    ...
    ##print("pred_xywh_size: ",pred_xywh.size)
